...
# Different from MultivariateGaussianClassifier because the covariance matrice is diagonal
# Could be done also by putting to zero the elements out of the diagonal after dot product
# In this was is more efficient (not visible of the n of feature is small)
class NaiveBayesClassifier:
    ...
    def estimate_params(self, c):
        ...
        N = D.shape[1]
        mu = vcol(D.mean(1))
        var = numpy.dot(D - mu, (D - mu).T) / N * numpy.eye(D.shape[0])
        return mu, var
